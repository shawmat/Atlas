...
_pathinfo_pat_atlas = re.compile(r'^/?(?P<l>\w.+)/(?P<s>\w.+)/(?P<p>\d+)/(?P<z>\d+)/(?P<x>-?\d+)/(?P<y>-?\d+)\.(?P<e>\w+)$')
...
def splitPathInfo(pathinfo):
    ...
    if _pathinfo_pat_atlas.match(pathinfo or ''):
        path = _pathinfo_pat_atlas.match(pathinfo)
