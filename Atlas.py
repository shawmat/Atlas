...
def get_tile(filename, coord, series, page):
    ...
    """
    #MS db = _connect(filename)
    db = _connect(filename + series + '.atlas')
    db.text_factory = bytes
    ...
    q = 'SELECT tile_data FROM tiles WHERE page_id = ? AND zoom_level=? AND tile_column=? AND tile_row=?'
    content = db.execute(q, (page, coord.zoom, coord.column, tile_row)).fetchone()
...
class Provider:
    ...
    def renderTile(self, width, height, srs, coord, series, page):
        """ Retrieve a single tile, return a TileResponse instance.
        """
        #MS mime_type, content = get_tile(self.tileset, coord)
        mime_type, content = get_tile(self.tileset, coord, series, page)
